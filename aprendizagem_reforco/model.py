...
import datetime
import time
from ambiente import Simulador as sim
...
class Normalizer():
    ...
    def observe(self, x):
        ...
        self.var = tf.clip_by_value(self.mean_diff/self.n, clip_value_min=1000, clip_value_max=50000)
...
class A2C:
	...
	def log(message):
		print('[Simulado em {:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()) + "] " + message)
	
	def optimizer(self):
		"""
		:return: Optimizer for your loss function
		"""
		return tf.train.AdamOptimizer(0.1).minimize(self.loss_val)        
